...
def astar(start, end):
    count = 0
    open_list = PriorityQueue()  # priority queue to fetch the smallest f valued node contains touple of (f value, node)
    ...
    while not open_list.empty():
        ...
        for neighbor in cur.neighbors:  # for every neighbor
            if neighbor.g > cur.g + 1:  # if its g value is greater than of current
                ...
                neighbor.g = cur.g + 1
                neighbor.h = h(neighbor.get_coordinates(), end.get_coordinates())
